...
namespace inventory_system
{
    public class ItemInInventory : MonoBehaviour
    {
        ...
        [SerializeField] private TMPro.TMP_Text t_countText;
        [SerializeField] private int currentCount = 0;

        [SerializeField] private GameObject newItemTag;
        [SerializeField] private Image I_selectItem;
        ...
        public void SelectItem()
        {
            if (controller != null && itemInfo != null)
            {
                ...
                I_selectItem.gameObject.SetActive(true);
            }
        }
        ...
        {
            selectble = false;
            I_selectItem.gameObject.SetActive(false);
        ...
        public void SetItemInfo(ItemTransaction newItem, bool newObj = true)
        {
            ...
            switch (slotType)
            {
                case SupportClass.InventorySlotType.mainCell:
                    I_itemImage.sprite = newItem.item.GetItemIcon_1x1();
                    break;
                case SupportClass.InventorySlotType.weaponCell:
                    I_itemImage.sprite = newItem.item.GetItemIcon_2x1();
            ...
            if (currentCount < 1)
            {
                ...
                t_countText.text = (currentCount <= 1) ? "" : currentCount.ToString();
                if (newObj) newItemTag.SetActive(true);
            }
            else
            {
                currentCount += (newItem.itemCount <= 1) ? 1 : newItem.itemCount;
                t_countText.text = currentCount.ToString();
            }
        }
